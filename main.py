...
eaten_monkey_counter = 0
died_monkey_counter = 0


def combined_monkey_thread():
    print("Starting combined_monkey_thread")
    global monkey_swim_stop_events, monkey_swim_threads, eaten_monkey_counter, died_monkey_counter
    ...
    while running and keep_running:
        ...
        if (current_time - last_island_check).seconds >= 10:
            print(f"{current_time} - Checking island events")
            for monkey in all_monkeys:
                if not monkey['in_sea'] and random.random() < 0.01:  # 1% chance
                    if monkey in all_monkeys:
                        ...
                        eaten_monkey_counter += 1
                        print(f'monkey eaten {eaten_monkey_counter}')
                        winsound.Beep(666, 950)
                        eaten_monkey_counter += 1
            last_island_check = current_time  # Moved outside of nested conditions, but still within the 10 second check
...
def render_scoop(monkey):
    ...
    # Check if the monkey is at sea
    if monkey != 'sea':
        return  # Exit the function if monkey is not at sea
    ...
    # Check if the monkey has been eaten
    if monkey['eaten']:
        return  # Exit the function if the monkey has been eaten
...
def points_15():
    global eaten_monkey_counter, died_monkey_counter
    flag_true= False
    if eaten_monkey_counter and died_monkey_counter ==1 and flag_true==False:
